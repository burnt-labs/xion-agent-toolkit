...
mod tests {
    ...
    // =========================================================================
    // list_treasuries tests
    // =========================================================================

    #[tokio::test]
    async fn test_list_treasuries_success() {
        let mock_server = MockServer::start().await;
        let admin_address = "xion1listadmin";
        let token = create_test_token(admin_address);

        Mock::given(method("GET"))
            .and(path_regex(
                r"/contract/xion1listadmin/xion/account/treasuries",
            ))
            .respond_with(ResponseTemplate::new(200).set_body_json(serde_json::json!([
                {
                    "contractAddress": "xion1treasury1",
                    "balances": {"uxion": "1000000"},
                    "block": {"height": "1000", "timeUnixMs": "1704067200000"},
                    "codeId": 1260,
                    "params": {
                        "redirect_url": "https://example.com/callback",
                        "icon_url": "https://example.com/icon.png",
                        "metadata": "{\"name\": \"Treasury One\"}"
                    }
                },
                {
                    "contractAddress": "xion1treasury2",
                    "balances": {"uxion": "2000000"},
                    "block": {"height": "2000", "timeUnixMs": "1704153600000"},
                    "codeId": 1260,
                    "params": {
                        "redirect_url": "https://example2.com/callback",
                        "icon_url": "https://example2.com/icon.png"
                    }
                }
            ])))
            .mount(&mock_server)
        ...
        let result = client.list_treasuries(&token).await;

        assert!(result.is_ok());
        let treasuries = result.unwrap();
        assert_eq!(treasuries.len(), 2);
        assert_eq!(treasuries[0].address, "xion1treasury1");
        assert_eq!(treasuries[0].balance, "1000000");
        assert_eq!(treasuries[0].name, Some("Treasury One".to_string()));
        assert_eq!(treasuries[1].address, "xion1treasury2");
        assert_eq!(treasuries[1].balance, "2000000");
        assert_eq!(treasuries[1].name, None);
    }

    #[tokio::test]
    async fn test_list_treasuries_empty() {
        let mock_server = MockServer::start().await;
        let admin_address = "xion1emptyadmin";
        let token = create_test_token(admin_address);

        Mock::given(method("GET"))
            .and(path_regex(
                r"/contract/xion1emptyadmin/xion/account/treasuries",
            ))
            .respond_with(ResponseTemplate::new(200).set_body_json(serde_json::json!([])))
            .mount(&mock_server)
            .await;
    ...
    #[tokio::test]
    async fn test_list_treasuries_with_multiple_balances() {
        let mock_server = MockServer::start().await;
        let admin_address = "xion1multibaladmin";
        let token = create_test_token(admin_address);

        Mock::given(method("GET"))
            .and(path_regex(
                r"/contract/xion1multibaladmin/xion/account/treasuries",
            ))
            .respond_with(
                ResponseTemplate::new(200).set_body_json(serde_json::json!([{
                    "contractAddress": "xion1multibal",
                    "balances": {
                        "uxion": "5000000",
                        "uusdc": "1000000"
                    }
        ...
        // Should return uxion balance
        assert_eq!(treasuries[0].balance, "5000000");
    }

    #[tokio::test]
    async fn test_list_treasuries_http_error() {
        let mock_server = MockServer::start().await;
        let admin_address = "xion1erroradmin";
        let token = create_test_token(admin_address);

        Mock::given(method("GET"))
            .and(path_regex(
                r"/contract/xion1erroradmin/xion/account/treasuries",
            ))
    ...
    // =========================================================================
    // query_treasury tests
    // =========================================================================

    #[tokio::test]
    async fn test_query_treasury_success() {
        let mock_server = MockServer::start().await;
        let admin_address = "xion1queryadmin";
        let treasury_address = "xion1treasuryquery";
        let token = create_test_token(admin_address);

        Mock::given(method("GET"))
            .and(path_regex(
                r"/contract/xion1queryadmin/xion/account/treasuries",
            ))
            .respond_with(
                ResponseTemplate::new(200).set_body_json(serde_json::json!([{
                    "contractAddress": treasury_address,
                    "balances": {"uxion": "10000000"},
                    "params": {
                        "redirect_url": "https://example.com/callback",
                        "icon_url": "https://example.com/icon.png",
                        "metadata": "{\"name\": \"Query Treasury\"}"
                    }
                }])),
            )
            .mount(&mock_server)
        ...
        let result = client
            .query_treasury(&token, treasury_address, QueryOptions::default())
            .await;

        assert!(result.is_ok());
        let treasury = result.unwrap();
        assert_eq!(treasury.address, treasury_address);
        assert_eq!(treasury.balance, "10000000");
        assert!(treasury.params.redirect_url.contains("example.com"));
    }

    #[tokio::test]
    async fn test_query_treasury_not_found() {
        let mock_server = MockServer::start().await;
        let admin_address = "xion1notfoundadmin";
        let treasury_address = "xion1notfoundtreasury";
        let token = create_test_token(admin_address);

        Mock::given(method("GET"))
            .and(path_regex(
                r"/contract/xion1notfoundadmin/xion/account/treasuries",
            ))
            .respond_with(ResponseTemplate::new(200).set_body_json(serde_json::json!([
                {
                    "contractAddress": "xion1othertreasury",
                    "balances": {"uxion": "1000"}
                }
            ])))
            .mount(&mock_server)
    ...
    // =========================================================================
    // query_contract_smart tests
    // =========================================================================

    #[tokio::test]
    async fn test_query_contract_smart_base64_response() {
        let mock_server = MockServer::start().await;
        let contract_address = "xion1contract123";

        // Create a base64-encoded response
        let inner_result = serde_json::json!({"balance": "999000"});
        let inner_json = serde_json::to_string(&inner_result).unwrap();
        let encoded_data = crate::treasury::api_client::helpers::base64_encode(&inner_json);

        Mock::given(method("GET"))
            .and(path_regex(
                r"/cosmwasm/wasm/v1/contract/xion1contract123/smart/",
            ))
            .respond_with(ResponseTemplate::new(200).set_body_json(serde_json::json!({
                "data": encoded_data
            })))
        ...
        assert_eq!(result_value["balance"], "999000");
    }

    #[tokio::test]
    async fn test_query_contract_smart_direct_json_response() {
        let mock_server = MockServer::start().await;
        let contract_address = "xion1contract456";

        // Direct JSON response (not base64 encoded)
        Mock::given(method("GET"))
            .and(path_regex(
                r"/cosmwasm/wasm/v1/contract/xion1contract456/smart/",
            ))
            .respond_with(ResponseTemplate::new(200).set_body_json(serde_json::json!({
                "data": {"permissions": ["burn", "mint"]}
            })))
            .mount(&mock_server)
            .await;
        ...
        let query_msg = serde_json::json!({"permissions": {}});
        let result = client
            .query_contract_smart(contract_address, &query_msg)
            .await;
        ...
        assert_eq!(
            ...
            serde_json::json!(["burn", "mint"])
        );
    }

    #[tokio::test]
    async fn test_query_contract_smart_http_error() {
        let mock_server = MockServer::start().await;
        let contract_address = "xion1contracterr";

        Mock::given(method("GET"))
            .and(path_regex(
                r"/cosmwasm/wasm/v1/contract/xion1contracterr/smart/",
            ))
            .respond_with(ResponseTemplate::new(404).set_body_string("Contract not found"))
            .mount(&mock_server)
            .await;
    ...
    #[tokio::test]
    async fn test_get_code_info_success() {
        let mock_server = MockServer::start().await;
        let code_id = 1260u64;

        // Mock the RPC response
        Mock::given(method("GET"))
            .and(path(format!("/cosmwasm/wasm/v1/code/{}", code_id)))
            .respond_with(ResponseTemplate::new(200).set_body_json(serde_json::json!({
                "code_info": {
                    "code_id": "1260",
                    "creator": "xion1creator123",
                    "data_hash": "ABCDEF0123456789ABCDEF0123456789ABCDEF0123456789ABCDEF0123456789"
                }
            })))
            .mount(&mock_server)
            .await;
        ...
        let result = client.get_code_info(code_id).await;

        assert!(result.is_ok());
        let info = result.unwrap();
        assert_eq!(info.code_id, code_id);
        assert_eq!(info.creator, "xion1creator123");
    ...
    #[tokio::test]
    async fn test_get_code_info_http_error() {
        let mock_server = MockServer::start().await;
        let code_id = 9999u64;

        Mock::given(method("GET"))
            .and(path(format!("/cosmwasm/wasm/v1/code/{}", code_id)))
            .respond_with(ResponseTemplate::new(404).set_body_string("Code not found"))
            .mount(&mock_server)
            .await;
    ...
    #[tokio::test]
    async fn test_get_code_info_uppercase_checksum() {
        let mock_server = MockServer::start().await;
        let code_id = 1261u64;

        // Return uppercase checksum
        Mock::given(method("GET"))
            .and(path(format!("/cosmwasm/wasm/v1/code/{}", code_id)))
            .respond_with(ResponseTemplate::new(200).set_body_json(serde_json::json!({
                "code_info": {
                    ...
                    "data_hash": "a1b2c3d4e5f6A1B2C3D4E5F6a1b2c3d4e5f6A1B2C3D4E5F6a1b2c3d4e5f6A1B2C3D4"
                }
            })))
            .mount(&mock_server)
            .await;
