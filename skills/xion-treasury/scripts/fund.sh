...
#
# Usage:
#   ./fund.sh <ADDRESS> <AMOUNT> [--network NETWORK] [--yes]
#
# Arguments:
...
#   ./fund.sh xion1abc... 1000000uxion --yes  # Skip confirmation
#
# Environment:
...
if [[ -z "$ADDRESS" ]]; then
  ...
  "usage": "./fund.sh <ADDRESS> <AMOUNT> [--network NETWORK] [--yes]"
}'
    exit 1
fi
...
if [[ "$SKIP_CONFIRM" != "true" ]] && [[ "$XION_SKIP_CONFIRM" != "true" ]]; then
  ...
  Network: $NETWORK"
    ...
    if ! confirm_operation "Fund Treasury" --show-details "$DETAILS"; then
        output_json '{
  "success": false,
...
RESULT=$("${CLI_CMD[@]}" treasury fund "$ADDRESS" "$AMOUNT" --network "$NETWORK" --output json 2>&1)
EXIT_CODE=$?

if [[ $EXIT_CODE -eq 0 ]]; then
    log_info "Treasury funded successfully"
    log_audit "FUND" "address=$ADDRESS amount=$AMOUNT network=$NETWORK"
    output_json "$RESULT"
else
